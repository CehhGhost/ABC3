...
.LFB6:
	...
	mov	rax, QWORD PTR -64[rbp]		# сохраняем argv[0]
	add	rax, 8				# для чтения с командной строки сдвигаем на argv[0]
	mov	rax, QWORD PTR [rax]
	lea	rsi, .LC0[rip]			# ссылка на 'r'
	mov	rdi, rax			# argv[1]
	call	fopen@PLT
	mov	QWORD PTR -24[rbp], rax		# FILE *input (= -24)
	cmp	QWORD PTR -24[rbp], 0		# сравнить input с NULL (input == NULL)
	...
	jmp	.L14
.L3:
	lea	rcx, -32[rbp]			# epsilon для передачи в fscanf(input, "%d%lf", &x, &epsilon);
	lea	rdx, -44[rbp]			# x для передачи в fscanf(input, "%d%lf", &x, &epsilon);
	mov	rax, QWORD PTR -24[rbp]		# input для передачи в fscanf(input, "%d%lf", &x, &epsilon);
	lea	rsi, .LC2[rip]
	mov	rdi, rax
	mov	eax, 0
	call	__isoc99_fscanf@PLT
	mov	rax, QWORD PTR -24[rbp]		# input для передачи в fclose(input);
	mov	rdi, rax
	call	fclose@PLT
	mov	rax, QWORD PTR -64[rbp]		# сохраняем argv[0]
	...
	jmp	.L14
.L5:
	mov	rdx, QWORD PTR -32[rbp]		# epsilon для передачи в get_chx(x, epsilon);
	mov	eax, DWORD PTR -44[rbp]		# x для передачи в get_chx(x, epsilon);
	movq	xmm0, rdx
	mov	edi, eax
	call	get_chx@PLT
	mov	rax, QWORD PTR -16[rbp]		# output для передачи в fprintf(output, "Your ch(x) = %lf\r\n", get_chx(x, epsilon));
	lea	rsi, .LC4[rip]
	mov	rdi, rax
	mov	eax, 1
	call	fprintf@PLT
	mov	rax, QWORD PTR -16[rbp]		# output для передачи в fclose(input);
	mov	rdi, rax
	call	fclose@PLT
	mov	eax, 0
	jmp	.L14
.L2:
	lea	rdi, .LC5[rip]
	call	puts@PLT
	lea	rax, -36[rbp]			# flag для передачи в scanf("%d", &flag);
	mov	rsi, rax
	lea	rdi, .LC6[rip]
	mov	eax, 0
	...
	mov	eax, 0
	call	printf@PLT
	lea	rax, -44[rbp]			# x для передачи в scanf("%d", &x);
	mov	rsi, rax
	lea	rdi, .LC6[rip]
	mov	eax, 0
	call	__isoc99_scanf@PLT
...
.L6:
	lea	rdi, .LC14[rip]
	call	puts@PLT
	lea	rax, -40[rbp]			# seed для передачи в scanf("%d", &seed);
	mov	rsi, rax
	lea	rdi, .LC6[rip]
	mov	eax, 0
	call	__isoc99_scanf@PLT
